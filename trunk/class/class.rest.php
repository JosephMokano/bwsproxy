    ...
    function getServiceResponse($param){
	    ...
	    do {
		    ...
		    if (preg_match('/<!--/',$line))
		     ...
		     if (preg_match('/-->/',$line))
		    	$start = true;		   
		    if ($start and !preg_match('/xml-stylesheet/',$line) and !preg_match('/<!--/',$line) and !preg_match('/-->/',$line) and (preg_match('/^ /',$line) or preg_match('/^</',$line) or preg_match('/>/',$line) or (($param['bwsp_service'] == 'partsRegistry_das') or (strlen($line) >40 and !preg_match('/Content-Type/',$line) and !preg_match('/Last-Modified/',$line) and !preg_match('/internal.sanger.ac.uk/',$line))) ) ){
		    	if (!preg_match('/^</',$line))
